...
// Ejemplo de uso de la función para verificar la asistencia diaria
const userId = "61234567890"; // ID del usuario
const fecha = new Date("2023-05-23"); // Fecha a verificar
verificarAsistenciaDiaria(userId, fecha)
  .then((resultado) => console.log(resultado))
  .catch((error) => console.error(error));

// Ejemplo de uso de la función para verificar la asistencia mensual
const mes = 5; // Mes (1-12)
const anio = 2023; // Año
verificarAsistenciaMensual(userId, mes, anio)
  .then((asistenciaCompleta) => {
    if (asistenciaCompleta) {
      console.log("El brigadista tiene derecho al bono de asistencia");
      // Aquí puedes agregar el código para otorgar el bono de asistencia al brigadista
    } else {
      console.log("El brigadista no tiene derecho al bono de asistencia");
    }
  })
  .catch((error) => console.error(error));
